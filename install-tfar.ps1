...
if ($TFAR_Zero -and $TS_Running) {
    Write-Host -ForegroundColor Red "[FAIL]"
    Write-Host -ForegroundColor Yellow " > TFAR 0.x installé et Teamspeak en cours d'exécution"
    exit
}
elseif ($TFAR_Zero) {
...
if (!$TFAR_Beta -and !$TFAR_Zip) {
    Write-Host -ForegroundColor Red "[FAIL]"
    Write-Host -ForegroundColor Yellow " > Plugin pas installé et setup mmanquant"
    Write-Host -ForegroundColor Yellow " > Veuillez vérifier l'intégrité des mods téléchargés !"
    exit
} elseif(!$TFAR_Beta) {
